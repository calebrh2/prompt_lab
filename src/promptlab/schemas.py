...
def schema_description(model: type[BaseModel]) -> str:
    """Return the model's JSON Schema for embedding in a prompt."""
    return json.dumps(model.model_json_schema(), indent=2)


def render_prompt(template: str, model: type[BaseModel], document_text: str) -> str:
    """Fill prompt placeholders from the live schema. Do not use str.format()."""
    return template.replace("{schema_description}", schema_description(model)).replace(
